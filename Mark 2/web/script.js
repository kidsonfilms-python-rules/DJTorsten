...
async function getJSEmailPassword(email_id, password_id) {
    ...
    eel.getEmailPassword(email, password)
}
...
async function switchOnefile(active) {
    if (active) {
...
function setState(newState) {
    ...
    switch (newState) {
        case state.ready:
            ...
            document.getElementById('convert').innerHTML = "Start DJTorsten";
...
async function checkDashboard(chkDashboard) {
    checkBox = document.getElementById(chkDashboard);
    if (checkBox == 'true') {
        checkBoxData = 'true'
    } else {
        checkBoxData = 'false'
    ...
    eel.getDashboardBool(checkBoxData)
}

async function startStopScript() {
  ...
  if (x.innerHTML === " ") {
    ...
    x.innerHTML = " ";
    y.innerText = "Start DJTorsten";
    eel.stopScript()
    y.style.backgroundColor = "#458BC6"
  }
